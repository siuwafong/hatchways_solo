...
import { Dialog, DialogTitle, DialogContent, DialogActions, DialogContentText, TextField, Button, IconButton, Grid, List, Tabs, Tab }from '@material-ui/core';
...
const InvitationDialog = ( {setFilteredFriends, friends, setFriends, letOpen} ) => {
    ...
    return (
        <Dialog fullWidth={true} onClose={handleClose} open={open} className="friendInvite">
            <Tabs
                value={tabValue}
            ...
            <React.Fragment>
                ...
                <DialogContent>
                    ...
                    <DebounceInput
                        ...
                        margin="dense"
                        value={friendEmail}
                        helperText={validEmail === false ? "Please enter a valid email address" : "" }
            ...
            <React.Fragment>
                ...
                {invites.received.filter(contact => contact.status !== "declined").length !== 0 
                ?
                <DialogContent>
                    {invites.received.filter(contact => contact.status === "pending").map(contact => (
                        <FoundContact
                            name={contact.name}
                            image={contact.image}
                            id={contact.id}
