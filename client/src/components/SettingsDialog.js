...
const formReducer = (state, action) => {
    switch(action.type) {
        ...
        case 'image':
            if (action.payload.name.split(".").pop() === "jpg" || action.payload.name.split(".").pop() === "png" || action.payload.name.split(".").pop() === "jpeg") {
...
const SettingsDialog = ({ name, password, image, language, letOpen, currentUser, setCurrentUser }) => {

    const [open, setOpen] = useState(letOpen)
    const [showPasswordSettings, setShowPasswordSettings] = useState(false)
    const [formState, formDispatch] = useReducer(formReducer, initialFormState, initFunc)
    ...
    const handleSubmit = () => {
        ...
        if (formState.password.value !== formState.confirmPassword.value) {
            ...
            return
        } else if (formState.password.isValid === false || formState.confirmPassword.isValid === false) {
        ...
        if ((formState.password.isValid === true && formState.password.value === formState.confirmPassword.value)) {
            let body = {}
            ...
            if (Object.keys(body).length !== 0) {
                console.log(body)
                axios.post(`http://${url}/user/${userId}/updateprofile`, body, {
                    headers: {
        ...
        if (formState.image.isValid === true && formState.image.error === false) {
            ...
            axios.post(`http://${url}/user/${userId}/updateprofileimg`, data, {
                headers: {
            ...
            <DialogContent>
                ...
                >
                    {languages.sort().map(language => (
                        <option value={language} key={uuidv4()}>{language}</option>
                    ))}
                </NativeSelect>
            </DialogContent>
            <DialogActions>
